...
func transferShopItem(from, to *world.PlayerInfo, item *world.InvItem, count int32, deps *Deps) {
	...
	if item.Stackable && item.Count > count {
		...
		to.Dirty = true
		sendAddItem(to.Session, newItem, info)
	}
}
...
func transferShopGold(from, to *world.PlayerInfo, amount int32, deps *Deps) {
	...
	if fromAdena.Count <= 0 {
		...
		sendRemoveInventoryItem(from.Session, fromAdena.ObjectID)
	} else {
		sendItemCountUpdate(from.Session, fromAdena)
	}

	// 給目標增加
	info := deps.Items.Get(world.AdenaItemID)
	...
	to.Dirty = true
	sendAddItem(to.Session, toAdena, info)
}
